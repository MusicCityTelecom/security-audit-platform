...
public sealed class JobScheduler : BackgroundService
{
    ...
    public JobScheduler(IModuleRegistry modules, IEnumerable<IExecutionProvider> providers, PlatformDatabase database)
    {
        _modules = modules;
        _providers = providers.ToDictionary(x => x.Runtime);
        _database = database;
    }
    ...
    public Job Enqueue(string moduleId, string target, Guid? engagementId, bool confirmed)
    {
        ...
        var errors = module.ValidationIssues.Where(x => x.IsError).ToArray();
        if (errors.Length > 0) throw new InvalidOperationException("Module is invalid and cannot execute.");
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        await foreach (var queued in _queue.Reader.ReadAllAsync(stoppingToken))
        {
            ...
            try
            {
                if (!_providers.TryGetValue(module.Manifest.Runtime, out var provider)) throw new InvalidOperationException($"No execution provider for runtime {module.Manifest.Runtime}.");
                var plan = new ModuleExecutionPlan(module.Manifest.Id, module.Manifest.Runtime, module.Manifest.Entrypoint, [queued.Target], module.Directory, module.Manifest.NetworkBehavior, false);
                ...
                var completed = running with { State = state, FinishedAt = result.FinishedAt, ExitCode = result.ExitCode, Error = string.IsNullOrWhiteSpace(result.StandardError) ? null : result.StandardError[..Math.Min(result.StandardError.Length, 4000)] };
