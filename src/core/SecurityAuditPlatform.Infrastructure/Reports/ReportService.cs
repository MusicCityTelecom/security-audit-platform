...
public sealed class ReportService
{
    ...
    public string BuildHtml(string title)
    {
        ...
        var findingRows = string.Join("", findings.Select(f =>
            $"<tr><td>{E(f.Severity.ToString())}</td><td>{E(f.Title)}</td><td>{E(f.Asset ?? "")}</td><td>{E(f.Description)}</td></tr>"));
        ...
        return "<!doctype html><html><head><meta charset="utf-8"><title>" +
            ...
            "th,td{border:1px solid #ccd2dc;padding:8px;text-align:left}" +
            "th{background:#eef1f6}</style></head><body><h1>" +
            E(title) +
            "</h1><p>Generated " +
            E(DateTimeOffset.UtcNow.ToString("u")) +
            "</p><h2>Findings (" +
            findings.Count +
            ")</h2><table><tr><th>Severity</th><th>Title</th><th>Asset</th><th>Description</th></tr>" +
            findingRows +
            "</table><h2>Jobs (" +
            jobs.Count +
            ")</h2><table><tr><th>Module</th><th>Target</th><th>State</th><th>Created</th></tr>" +
            jobRows +
            "</table></body></html>";
    }
}
