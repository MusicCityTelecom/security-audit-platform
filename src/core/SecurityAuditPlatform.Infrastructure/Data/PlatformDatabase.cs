...
CREATE TABLE IF NOT EXISTS engagements (
  id TEXT PRIMARY KEY, name TEXT NOT NULL, created_at TEXT NOT NULL, expires_at TEXT NULL
);
CREATE TABLE IF NOT EXISTS scope_targets (
  engagement_id TEXT NOT NULL, value TEXT NOT NULL, excluded INTEGER NOT NULL DEFAULT 0,
  PRIMARY KEY (engagement_id, value, excluded),
  FOREIGN KEY (engagement_id) REFERENCES engagements(id) ON DELETE CASCADE
);
CREATE TABLE IF NOT EXISTS jobs (
  id TEXT PRIMARY KEY, module_id TEXT NOT NULL, engagement_id TEXT NULL, target TEXT NOT NULL,
  state INTEGER NOT NULL, created_at TEXT NOT NULL, started_at TEXT NULL, finished_at TEXT NULL,
  exit_code INTEGER NULL, error TEXT NULL
);
...
";
    ...
    public void SaveJob(Job job)
    {
        ...
        cmd.CommandText = @"INSERT INTO jobs(id,module_id,engagement_id,target,state,created_at,started_at,finished_at,exit_code,error)
VALUES($id,$module,$engagement,$target,$state,$created,$started,$finished,$exit,$error)
ON CONFLICT(id) DO UPDATE SET state=$state,started_at=$started,finished_at=$finished,exit_code=$exit,error=$error;";
        cmd.Parameters.AddWithValue("$id", job.Id.ToString());
        cmd.Parameters.AddWithValue("$module", job.ModuleId);
        cmd.Parameters.AddWithValue("$engagement", (object?)job.EngagementId?.ToString() ?? DBNull.Value);
        cmd.Parameters.AddWithValue("$target", job.Target);
        ...
        cmd.Parameters.AddWithValue("$finished", (object?)job.FinishedAt?.ToString("O") ?? DBNull.Value);
    ...
    {
        using var c = Open(); using var cmd = c.CreateCommand();
        cmd.CommandText = "SELECT id,module_id,engagement_id,target,state,created_at,started_at,finished_at,exit_code,error FROM jobs ORDER BY created_at DESC LIMIT $limit;";
        ...
        while (reader.Read())
        {
            jobs.Add(new Job(
                ...
                (JobState)reader.GetInt32(4), DateTimeOffset.Parse(reader.GetString(5)),
